...
# Configuring & Tuning the Operating System 

* [Usable Ports by Kernel](#usable-ports-by-kernel)
...
## Usable Ports by Kernel

To let the Kernel use the up most ports, the recommendation is to make all ports usable and reserve the nominated services ports. That can be achieved by applying configurations to the `/etc/sysctl.conf`.
